import {
  ...
  HostListener, NgZone, OnDestroy, OnInit, ViewChild
} from '@angular/core';
...
import { PageLoderComponent } from '../../shared/components/page-loder/page-loder.component';
...
@Component({
  ...
  imports: [
    ...
    NgFor, NgStyle, NgIf,
    FormsModule,
    CatfishHeaderComponent,
    PageLoderComponent
  ],
  templateUrl: './catfish.component.html',
  styleUrl: './catfish.component.scss'
})
export class CatfishComponent implements OnInit, OnDestroy {
  ...
  showPageLoader = !sessionStorage.getItem('catfish_loaded');
  ...
  constructor(
    ...
    private zone: NgZone,
    private catfishSoundService: CatfishSoundService,
    public userService: UserService
  ) {
    // Map user$ observable to just the balance number
    this.userBalance$ = new Observable<number>(observer => {
      return this.userService.user$.subscribe(u => observer.next(u.balance));
    });
  }

  async ngOnInit() {
    this.balanceSub = this.userService.user$.subscribe(u => {
      this._currentBalance = u.balance;
      this.cdr.markForCheck();
    });

    if (this.showPageLoader) {
      this.loadingProgress = 0;
      this.loadingMessage = 'INITIALIZING SYSTEM...';
      this.cdr.detectChanges();

      // Wait one tick so Angular renders the loader component first,
      // then start loading — otherwise loader hasn't initialized yet
      await new Promise(resolve => setTimeout(resolve, 50));

      await this.preloadAssets();
      // showPageLoader = false is handled by onPageLoaded() after fade-out
    }
  }
  ...
  onPageLoaded() {
    sessionStorage.setItem('catfish_loaded', '1');
    this.showPageLoader = false;
    this.cdr.detectChanges();
  }

  private preloadAssets(): Promise<void> {
    const images = this.baseProfiles.map(p => p.image);
    const audios = [
      'assets/audios/catfish/backgroundAudio.mp3',
      'assets/audios/catfish/cashout.mp3',
      'assets/audios/catfish/catfished.mp3',
      'assets/audios/catfish/swep.mp3'
    ];

    const allAssets = [...images, ...audios];
    ...
    const messages = [
      ...
      'LOADING PROFILES...',
      'PREPARING DECKS...',
      'CALIBRATING CATFISH RADAR...',
      'WARMING UP THE SWIPE ENGINE...',
      'READY TO SWINDLE...'
    ];

    const onProgress = () => {
      loaded++;
      this.loadingProgress = Math.min(Math.round((loaded / total) * 100), 99); // hold at 99 until fully done
      const msgIdx = Math.min(
        Math.floor((this.loadingProgress / 100) * messages.length),
        messages.length - 1
      );
      this.loadingMessage = messages[msgIdx];
      this.cdr.detectChanges();
    };
    ...
    const imagePromises = images.map(src =>
      new Promise(resolve => {
        const img = new Image();
        img.onload = () => { onProgress(); resolve(true); };
        img.onerror = () => { onProgress(); resolve(true); };
        img.src = src;
      })
    );
    ...
    const audioPromises = audios.map(src =>
      new Promise(resolve => {
        ...
        audio.oncanplaythrough = () => { onProgress(); resolve(true); };
        audio.onerror = () => { onProgress(); resolve(true); };
        audio.src = src;
        audio.load();
      })
    );
    ...
    return Promise.all([...imagePromises, ...audioPromises]).then(() => {
      ...
      this.loadingMessage = 'READY TO SWINDLE...';
      this.cdr.detectChanges();
    });
  }
  ...
  dismissAlert() {
    this.showInsufficientAlert = false;
    this.cdr.detectChanges();
  }
  ...
  playSound(type: 'cashout' | 'catfished' | 'swep' | 'background') {
    ...
    if (type === 'cashout') audio = this.cashoutAudioRef?.nativeElement;
    else if (type === 'catfished') audio = this.catfishedAudioRef?.nativeElement;
    else if (type === 'swep') audio = this.swepAudioRef?.nativeElement;
    else audio = this.backgroundAudioRef?.nativeElement;
  ...
  // ── Start Game — deduct bet first ─────────────────────────
  startGame() {
    if (this.betAmount > this._currentBalance) {
    ...
    const deducted = this.userService.deductBet(this.betAmount);
    if (!deducted) {
      this.showInsufficientAlert = true;
      this.cdr.detectChanges();
      setTimeout(() => this.dismissAlert(), 3000);
  ...
  onCashout() {
    ...
    // Credit the cashout amount to balance
    this.userService.creditWinnings(this.currentCashoutAmount);
  ...
  onSoundChanged(event: any) {
    ...
    if (!enabled) {
      this.catfishSoundService.stopAll([
        this.backgroundAudioRef, this.cashoutAudioRef,
        this.catfishedAudioRef, this.swepAudioRef
      ]);
    } else {
  ...
  private addToHistory() {
    ...
    const multiplier = this.resultType === 'cashout'
      ? (this.resultAmount / this.betAmount).toFixed(2) : '0.00';
    this.betHistory.unshift({
      time, betAmount: this.betAmount,
      catfish: this.catfishCount, result,
      payout: 'x' + multiplier
    });
